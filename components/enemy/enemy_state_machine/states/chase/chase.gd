...
func _rotate_to_face_destination(destination: Vector2, delta: float) -> void:
	...
	var rotation_amount: float = sign(angle_to) * min(delta * enemy_parent.rotation_speed * 2, abs(angle_to))
...
func override_physics_process(_delta: float) -> void:
	if enemy_parent.lightsource.light_hitting_player and enemy_parent.lightsource.lightsource_active:
		var direction: Vector2 = player.global_position - enemy_parent.global_position
		if abs(enemy_parent.transform.x.angle_to(direction)) > 0.05:
			_rotate_to_face_destination( player.global_position, _delta )
			return

		if enemy_parent.global_position.distance_squared_to(player.global_position) > 5 :
			var direction_to_destination: Vector2 = enemy_parent.global_position.direction_to(player.global_position).normalized()
			enemy_parent.velocity = enemy_parent.chasing_movement_speed * direction_to_destination
